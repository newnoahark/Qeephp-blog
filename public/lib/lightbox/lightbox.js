...
(function (pluginNamespace) {
    ...
    var Plugin = (function () {
        ...
        //合并用户定义参数
        Plugin.prototype.option = function (options) {
            $.extend(this.options, options);
        };
        // 图像数量标签
        Plugin.prototype.imageCountLabel = function (currentNumber, totalImages) {
        ...
        Plugin.prototype.enable = function () {
            var self = this;
            // 绑定点击事件,并过滤一些属性
            $('body').on('click', 'a[rel^=lightbox], area[rel^=lightbox], a[data-lightbox], area[data-lightbox]', function (evt) {
                //调用开始图片预览方法
        ...
        Plugin.prototype.build = function () {
            ...
            this.lightbox.hide();
            this.lightbox.hide().on("click", function (evt) {
        ...
        //显示图片,如果图片是一组就显示一组图片
        Plugin.prototype.start = function (link) {
            var _this = this;
            var self = this;
            ...
            var imageNumber = 0;
            //添加到图像数组内
            var addToAlbum = function (lk) { _this.album.push({ link: lk.attr("href"), title: lk.attr("data-title") || lk.attr("title") }); };
        ...
        // 改变显示图片
        Plugin.prototype.changeImage = function (imgNumber) {
            var self = this;
            // 禁用键盘导航
        ...
        // 尺寸容器
        Plugin.prototype.sizeContainer = function (imgWidth, imgHeight) {
            var self = this;
            //边框宽度/高度
            var oldWidth = this.outerContainer.outerWidth();
            var oldHeight = this.outerContainer.outerHeight();
            // 图像宽度/高度 +padding
            var newWidth = imgWidth + this.containerLeftPadding + this.containerRightPadding;
            var newHeight = imgHeight + this.containerBottomPadding + this.containerTopPadding;
            //定位显示文字和按钮位置
            function posResize() {
                //设置文字容器的大小
        ...
        //更新导航
        Plugin.prototype.updateNav = function () {
            //检查浏览器是否支持触摸操作
        ...
        //更新内容说明区域
        Plugin.prototype.updateDetails = function () {
            var self = this;
            //定位内容描点
            ...
            //内容显示动画  
            self.lightbox.find(".lb-dataContainer").fadeIn(self.options.resizeDuration, function () {
                return self.sizeOverlay();
            });
        };
        //预加载相邻图片
        Plugin.prototype.preloadNeighboringImages = function () {
        ...
        // 拉伸以覆盖.以适应视图
        Plugin.prototype.sizeOverlay = function () {
            this.overlay.width($(document).width()).height($(document).height());
        };
        //    启用键盘导航
        Plugin.prototype.enableKeyboardNav = function () {
            $(document).on('keyup.keyboard', $.proxy(this.keyboardAction, this));
        };
        //禁用键盘导航
        Plugin.prototype.disableKeyboardNav = function () {
            $(document).off(".keyboard"); //移除键盘事件
        };
        //键盘激活
        Plugin.prototype.keyboardAction = function (evt) {
            //键盘操作代码
            var KEYCODE_ESC = 27;
